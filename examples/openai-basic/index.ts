...
import { gateRunToolsParams, gateTool, gateToolExecutors } from '@miodragmtasic/agentgate-openai';
...
async function main() {
	...
	const firstTool = gatedParams.tools[0];
	const updateFn = firstTool?.function.function;
	if (!updateFn) {
		throw new Error('Expected gateRunToolsParams() to produce an executable tool wrapper.');
	}
	...
	const queryDatabase = gatedExecutors.query_database;
	const manageUsers = gatedExecutors.manage_users;
	if (!queryDatabase || !manageUsers) {
		throw new Error('Expected gated executors for query_database and manage_users.');
	}
	...
	const manageResult = await manageUsers({
		name: 'new_user',
		role: 'viewer',
	});
