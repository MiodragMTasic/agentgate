...
import { AgentGate, ConsoleTransport, consoleSink } from '@miodragmtasic/agentgate-core';
import type {
	ApprovalRequest,
	ApprovalResponse,
	HITLTransport,
...
const emailTool = gateTool(gate, {
	...
	identity: agentIdentity,
	run: async (input) => JSON.stringify({ sent: true, to: input.to, subject: input.subject }),
});
...
const deleteTool = gateTool(gate, {
	...
	identity: agentIdentity,
	run: async (input) => JSON.stringify({ deleted: true, accountId: input.accountId }),
});
...
async function main() {
	console.log('\n=== AgentGate — Human-in-the-Loop Approval Example ===');
	console.log(
		`Mode: ${USE_INTERACTIVE ? 'INTERACTIVE (you approve in terminal)' : 'AUTO-APPROVE (demo)'}`,
	);
