...
import { AgentGate, consoleSink, parsePolicyFromFile } from '@miodragmtasic/agentgate-core';
...
async function main() {
	...
	const tools = runner.wrapTools([
		{
			name: 'get_weather',
			run: async (input: unknown) => JSON.stringify({ temp: 18, condition: 'cloudy' }),
		},
		{
			name: 'read_file',
			run: async (input: unknown) => JSON.stringify({ content: 'file contents here' }),
		},
	]);

	// This should be allowed (agent role + get_weather)
	const weatherRunner = tools[0];
	const fileRunner = tools[1];
	if (!weatherRunner || !fileRunner) {
		throw new Error('Expected wrapped Anthropic tools for get_weather and read_file.');
	}

	const result1 = await weatherRunner.run({ location: 'London' });
	console.log('Weather via runner:', result1);

	// This should be denied (agent role can't use read_file — admin only)
	const result2 = await fileRunner.run({ path: '/data/report.csv' });
